...
//	}
	...
	public String getList(@PathVariable("pageno") int pageno, HttpServletRequest hsr, Model model) {
		...
		if (pageno == 1) {
			...
			pDirection = "<a href='/app/list/" + (pageno - 1) + "'>이전페이지</a>" + "<a href='/app/list/" + (pageno + 1)
					+ "'>다음페이지</a>";
		}
...
//	}
	...
	/*
	 * @RequestMapping(value = "/save",method = RequestMethod.POST)//보여지는 jsp from의
	 ...
	 * bbs=sqlSession.getMapper(iBBS.class);//인터페이스 호출 준비 bbs.writebbs(pTitle,
	 * pContent, pWriter, pPasscode);//인터페이스 paramiter 전달 return "redirect:/list/1";
	 * }
	 */
